...
import { latestJournals } from '../../data/dummyData';

const JournalDetails = () => {
  const { id } = useParams();
  const journal = latestJournals.find(j => j.id === parseInt(id));
          ...
          <div className="flex flex-col lg:flex-row gap-12">
            ...
            <div className="lg:w-1/3 shrink-0">
              ...
              >
                ...
                <img src={journal.image} alt={journal.title} className="w-full h-auto aspect-[3/4] object-cover group-hover:scale-105 transition-transform duration-700" />
                <div className="absolute inset-0 bg-gradient-to-t from-gray-900/60 via-transparent to-transparent"></div>
              </motion.div>
              
              {/* Action Buttons (Desktop Sidebar) */}
              <div className="hidden lg:flex flex-col gap-3 mt-6">
                <button className="w-full flex items-center justify-center gap-2 bg-red-50 hover:bg-red-600 hover:text-white text-red-600 font-bold py-3.5 rounded-xl transition-colors border border-red-100 shadow-sm">
              ...
              <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
                <div className="flex flex-wrap gap-2 mb-4">
                  ...
                  <span className="bg-gray-100 text-gray-600 text-xs font-bold px-3 py-1 rounded-md flex items-center gap-1 border border-gray-200">
                    <FaGlobeAmericas /> {journal.language}
                  </span>
                </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-y-4 gap-x-8 mb-8 bg-gray-50 p-6 rounded-2xl border border-gray-100">
                  <div className="flex items-start gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-sm font-semibold text-text">{journal.author}</p>
                    </div>
                  </div>
                  <div className="flex items-start gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-sm font-semibold text-text">{journal.institution}</p>
                    </div>
                  </div>
                  <div className="flex items-start gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-sm font-semibold text-text">{journal.publicationDate}</p>
                    </div>
                  </div>
                  <div className="flex items-start gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-sm font-semibold text-text">{journal.volume}, {journal.issue}</p>
                    </div>
                  </div>
                </div>
                ...
                <div className="inline-flex items-center gap-2 bg-black text-white px-4 py-2 rounded-lg text-sm font-medium mb-10 shadow-md">
                  <FaFingerprint className="text-accent" /> DOI: <span className="font-mono tracking-wide">{journal.doi}</span>
                </div>
      ...
      <section className="py-16">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {latestJournals.filter(j => j.id !== journal.id).slice(0, 3).map(related => (
              <div key={related.id} className="bg-white rounded-2xl border border-gray-100 shadow-sm hover:shadow-xl transition-all overflow-hidden group flex flex-col h-full">
                ...
                <div className="p-5 flex flex-col flex-grow">
                  <h4 className="font-bold text-text mb-2 line-clamp-2 hover:text-primary transition-colors">
                    <Link to={`/journals/${related.id}`}>{related.title}</Link>
                  </h4>
                  <p className="text-xs text-light-text mb-4">{related.author} • {related.publicationDate}</p>
                  <div className="mt-auto border-t border-gray-100 pt-3">
                    <Link to={`/journals/${related.id}`} className="text-primary text-sm font-bold flex items-center gap-1 hover:text-accent transition-colors">
                      <FaEye /> View Details
                    </Link>
                  </div>
                </div>
